...
export default function CourseBuilderPage() {
  ...
  const handleSave = async (courseData: any) => {
    try {
      ...
      // Map Course structure → DB structure
      const mappedData = {
        ...
        image:
          courseData.image ||
          "https://images.unsplash.com/photo-1453733190148-c44698c26578",

        published: true,

        categoryId:
          courseData.location?.toLowerCase() === "london"
            ? "fine-cabinetry"
            : "green-woodworking",
        ...
        schedule:
          courseData.timeSlots?.join(", ") ||
          "Saturdays 09:30 - 16:30",

        // 🔥 CRITICAL FIX (RLS REQUIREMENT)
        creatorId: userId,
      };

      // 🔥 REAL DATABASE CALL
      const result = await createCourse(mappedData);
      ...
      setSuccessMsg(
        `"${mappedData.title}" has been successfully registered and published!`
      );
  ...
  return (
    <main
      className="min-h-screen bg-stone-50 py-10"
      id="course-builder-page-wrapper"
    >
